...
create_ssl_certificates() {
    ...
    if [[ -f "rootCA.key" ]] && [[ -f "rootCA.crt" ]]; then
        print_message "success" "Using existing Root CA"
...
EOF
    ...
    [[ "$INSTALLATION_MODE" == "remote" ]] && echo "  SSH User:          $SERVER_USER"
    echo "  IPv6:              $IPV6_ENABLED"
    echo "  Element Web:       $ELEMENT_ENABLED"
    echo "  Admin Username:    $ADMIN_USERNAME"
    echo "  SSL Certs:         $CREATE_SSL_CERTS"
    echo ""

...
#### Scenario: Root Key selection from files - back option
- **WHEN** user is presented with multiple Root Key files next to script
- **THEN** the system SHALL display option 0 labeled "Back to main menu"
- **AND** when user selects 0, return to main menu flow

#### Scenario: Root Key selection from certs - back option
- **WHEN** user is presented with multiple Root Keys in certs/
- **THEN** the system SHALL display option 0 labeled "Back to main menu"
- **AND** when user selects 0, return to main menu flow
...
#### Scenario: Main menu with Root Key available
- **WHEN** displaying menu with active Root Key
- **THEN** the system SHALL use the styled header format

#### Scenario: Main menu without Root Key
- **WHEN** displaying menu without active Root Key
- **THEN** the system SHALL use the styled header format
...
### Requirement: Root Key Configuration Prompts
Root Key configuration prompts (Organization, Country, State, City, Validity) SHALL be centralized in a single reusable function.

#### Scenario: Creating Root Key from menu
- **WHEN** user selects option to create new Root Key
...
- **WHEN** user completes Root Key configuration prompts
...
### Requirement: Root Key Creation Flow
Root Key creation from menu options SHALL use a unified flow that handles configuration prompting, confirmation, and creation.

#### Scenario: Create Root Key from option 8 (switch/create)
...
#### Scenario: Create Root Key from option 9
- **WHEN** user selects option 9 in menu_with_root_ca
- **THEN** the system SHALL call `create_root_ca_from_menu()` function
- **AND** follow the unified creation flow

#### Scenario: Create Root Key from menu without Root Key

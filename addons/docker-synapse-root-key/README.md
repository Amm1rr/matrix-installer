...
Docker Compose installer for Matrix Synapse with Private Key SSL certificates (Root Key from matrix-installer.sh).
...
This addon provides a fast, Docker Compose-based installation of Matrix Synapse using private key SSL certificates generated by matrix-installer.sh. It uses your custom Root Key for secure, private deployments without requiring public domain names or Let's Encrypt.
...
From matrix-installer.sh menu, select "Install private-key-docker-compose-synapse"
...
SSL Certificates:
  ...
  - Root Key: /opt/matrix/ssl/rootCA.crt
...
Element Web will require SSL trust. After installing Root Key:

- **Desktop browsers**: Restart browser after installing Root Key
- **Mobile apps**: Install Root Key in device settings
...
1. Verify Root Key is installed on client
2. Restart browser/app after installing Root Key
...
| **SSL** | Private Key (Root Key) | Let's Encrypt | Private Key (Root Key) |
...
This addon uses **Private Key SSL** from matrix-installer.sh. Certificates are:
- Generated by matrix-installer.sh Root Key
...
- `rootCA.crt` - Root Key certificate (for client distribution)
